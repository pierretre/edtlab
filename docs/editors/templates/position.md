...
pc: "PC1"  # Focused project (PC1-PC5). Makes this position appear on that project's page. Leave empty for a general/cross-project position.
location: "City, Country"
expectedStartDate: "Spring 2025"  # Expected start date - flexible format: "Spring 2025", "Q2 2025", "2025-06-01", etc.
publishedDate: 2024-01-15  # Date when the position was published in YYYY-MM-DD format
...
tags:
  - "digital-twins"  # Free-form keywords. Use "General" or "Extern" here if relevant — the project code goes in `pc` above, not here.
...
## Once the position is filled: switching to the ongoing-research view

When someone is hired for this position, don't delete the file — update it in place so it becomes the public record of the ongoing work. Add:
...
The page automatically switches from the "apply now" view to the ongoing-research
view (researcher, supervisors, status, dates) once `researcher` is filled in.
The `requirements`/`contacts`/apply-CTA fields are simply ignored at that point —
no need to remove them.

Optionally, also add `useCases` (references to related use cases) and
`publications` (a list of publication slugs) as the work progresses, and bump
