...
test.describe('Performance Tests', () => {
    ...
    test('Fonts load efficiently without FOIT', async ({ page, browserName }) => {
        ...
        const visibleTextElements = await page.locator('h1, h2, p').filter({ hasNot: page.locator('[style*="display: none"], [hidden], .sr-only') });
    ...
    test('Page works offline with service worker', async ({ page, context, browserName }) => {
        ...
        if (browserName === 'webkit') {
            test.skip('WebKit has known issues with offline functionality testing');
            return;
        }
        ...
        try {
            ...
            console.log('Offline test failed, this may be expected in some environments:', error.message);
        } finally {
